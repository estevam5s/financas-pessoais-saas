  ...
  return (
    <AppShell>
      <div className="space-y-8">
        <div>
          <h1 className="text-2xl font-semibold text-foreground">Olá, {greeting} 👋</h1>
          <p className="text-muted-foreground mt-1">Resumo das suas finanças — plano {isAdmin ? "Admin" : sub?.plan?.name || "Inicial"}.</p>
        </div>
        ...
        {!hasAccess && (
          <div className="rounded-xl border border-amber-500/30 bg-amber-500/5 p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div><p className="text-amber-300 font-medium">Seu período grátis terminou</p><p className="text-sm text-muted-foreground mt-1">Assine um plano para continuar registrando e liberar tudo.</p></div>
        ...
        <div className="grid sm:grid-cols-3 gap-4">
          <div className="rounded-xl border border-border bg-card p-5">
            <div className="flex items-center gap-2 text-muted-foreground text-xs uppercase tracking-wide"><Wallet className="size-4" /> Saldo total</div>
            <p className="text-2xl font-semibold text-foreground mt-2">{loaded ? money(saldo) : "…"}</p>
          </div>
          <div className="rounded-xl border border-border bg-card p-5">
        ...
        <div className="rounded-xl border border-border bg-card p-6">
          ...
          {!loaded ? <p className="text-muted-foreground text-sm">Carregando…</p> : tx.length === 0 ? (
            ...
            <div className="space-y-2">
              {tx.slice(0, 6).map((t) => (
                <div key={t.id} className="flex items-center justify-between p-3 rounded-lg hover:bg-white/5">
                  <div>
                    <p className="text-foreground text-sm font-medium">{t.description || t.category_name || "Lançamento"}</p>
                    <p className="text-xs text-muted-foreground">{t.category_name || "—"} · {new Date(t.occurred_at).toLocaleDateString("pt-BR")}</p>
                  </div>
