  ...
  return (
    <AppShell>
      <div className="space-y-8">
        <div><h1 className="text-2xl font-semibold text-foreground">Assinatura</h1><p className="text-muted-foreground mt-1 text-sm">Gerencie seu plano, pagamentos e reembolso.</p></div>
        ...
        <div className="rounded-2xl border border-border bg-card p-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-xl font-semibold text-foreground mt-1">{isAdmin ? "Admin (acesso total)" : sub?.plan?.name || "Inicial"}</p>
            <div className="flex flex-wrap gap-x-4 gap-y-1 mt-2 text-xs text-muted-foreground">
              {subscription?.status && <span>Status: <span className="text-muted-foreground">{subscription.status}</span></span>}
              {periodEnd && <span>Renova em: <span className="text-muted-foreground">{periodEnd}</span></span>}
          ...
          {subscription?.stripe_customer_id && <button onClick={portal} disabled={busy === "portal"} className="shrink-0 px-4 py-2 rounded-lg bg-white/10 hover:bg-white/20 text-foreground text-sm font-medium transition disabled:opacity-50">{busy === "portal" ? "Abrindo…" : "Gerenciar pagamento"}</button>}
        </div>
        ...
        <div className="flex items-center justify-between flex-wrap gap-3">
          ...
          <div className="inline-flex items-center gap-1 p-1 rounded-full border border-border bg-white/5">
            <button onClick={() => setCycle("month")} className={`px-4 py-1.5 rounded-full text-sm transition ${cycle === "month" ? "bg-emerald-500 text-white font-medium" : "text-muted-foreground"}`}>Mensal</button>
            <button onClick={() => setCycle("year")} className={`px-4 py-1.5 rounded-full text-sm transition flex items-center gap-1.5 ${cycle === "year" ? "bg-emerald-500 text-white font-medium" : "text-muted-foreground"}`}>Anual <span className="text-[10px] px-1.5 py-0.5 rounded-full bg-emerald-500/20 text-emerald-400">−20%</span></button>
          </div>
        </div>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-5">
          {plans.map((p: any) => {
            ...
            return (
              <div key={p.slug} className={`relative flex flex-col rounded-2xl border p-6 ${p.highlighted ? "border-emerald-500/50 bg-emerald-500/[0.06]" : "border-border bg-card"}`}>
                ...
                <div className="mt-3 flex items-end gap-1"><span className="text-3xl font-bold text-foreground">{free ? "R$ 0" : brl(cycle === "year" ? Math.round(price / 12) : price)}</span>{!free && <span className="text-muted-foreground text-sm mb-1">/mês</span>}</div>
                {!free && cycle === "year" && <p className="text-xs text-muted-foreground mt-1">{brl(price)}/ano</p>}
                <button onClick={() => checkout(p.slug)} disabled={free || isCurrent || busy === p.slug} className={`mt-5 w-full py-2.5 rounded-lg text-sm font-medium transition disabled:cursor-not-allowed ${isCurrent ? "bg-white/5 text-muted-foreground" : p.highlighted ? "bg-emerald-500 text-white hover:bg-emerald-600" : "bg-white/10 text-white hover:bg-white/20"}`}>{busy === p.slug ? "Aguarde…" : isCurrent ? "Plano atual" : free ? "Incluído" : "Assinar"}</button>
                <ul className="mt-6 space-y-2.5 text-sm flex-1">{(p.features || []).map((f: string, i: number) => (<li key={i} className="flex gap-2 text-muted-foreground"><Check className="h-4 w-4 text-emerald-400 shrink-0 mt-0.5" /><span>{f}</span></li>))}</ul>
              </div>
            )
          })}
        </div>
        <p className="text-xs text-muted-foreground">Pagamentos via Stripe. Cancele quando quiser. Reembolso de 7 dias. Sem cobrança por armazenamento.</p>
      </div>
    </AppShell>
  )
}
